...
try {
    ...
    $error_dir     = Join-Path $repo_dir "error"
    ...
    if (-not (Test-Path $error_dir)) {
        New-Item -ItemType Directory -Path $error_dir -Force | Out-Null
    }
    ...
    $py_args.Add($error_dir) | Out-Null
    ...
    $formats = Read-Host "Enter output formats (json,csv,html,yaml,db) or leave blank for all"
